   ...
   \begin{itemize}
      ...
      \item
         ...
         \begin{itemize}
            ...
            \item
               $\displaystyle\sum_{v\ s.t.\ F(v)=1\wedge G(v)=0} D(v) < h^{-1}$ (false
               negatives have a low probability to be sampled)
         \end{itemize}
   \end{itemize}
\end{definition}
...
\begin{algorithm}
\begin{enumerate}
  \item Initialise $g$ to the conjunction of all possible disjunctions of at most $k$ literals (there are $O(n^k)$ such disjunctions),
...
Alg.~\ref{algCNF} is used in the proof by Valiant. In this algorithm, the initialization of the learned function $g$ to the false constraint expressed as the conjunction of all possible disjunctions
leads to the learning of a minimal  generalization of the positive examples.

\sylvain{well, it has nothing to do with minimality\dots}
...
The \emph{degree} of a Boolean formula is the largest number of prime
implicants in an equivalent formula that is written as a non-redundant sum of
prime-implicants.

Proof of this theorem relies on Alg.~\ref{algDNF}. As previously, the algorithm guarantees that a minimal generalization is learned, up to the approximations previously defined.
...
\section{Boolean functions in biology}

In this section, we will present three boolean formalisms used in biology, and to which we will later whether or not PAC learning is applicable.
...
\sylvain{TODO: add a paragraph explaining how this fits into DNF for
	PAC-learning}

\subsection{$k$-CNF representations of influence systems}

If one wishes to deal with influence systems containing inhibitors, the monotone DNF representation is no longer a fit.

Instead, one may switch to the $k$-CNF representation of the systems, which constrains not the presence of inhibitors but the number of species that can play a given "role". To be more precise, let's consider the following CNF for a hypothetic influence:
...
Then each of the clause can be interpreted as a role, and for the activation of our hypothetic gene, we need at least one specie that plays this role. For example, the enzyme can be $a$, $b$, or $c$ whereas 
...
In this case, the Alg.~\ref{algCNF} is repeated $n$ times in order to learn each of the $f_i$.
...
\sylvain{what should be here are the results of the ``true'' sample/oracle
based learning on the example}
...
\textsc{Sample} and \textsc{Oracle}, but we restrict ourselves to the observations that can be obtained from data time-series.
...
From now on, we can hence no longer pretend to recover {\`a} la Thomas networks but we will instead focus on finding the activation and deactivation functions for each genes, which are defined below.

\sylvain{this explanation of why we choose activation/deactivation does not
convince me}
...
\sylvain{This is the main topic! It should be more than 2 lines long}
