...
Note that it is possible to use two different parameters $h_1$ an $h_2$ for the \emph{probability of false negatives} and the \emph{quality of the approximation}, but here, we used $h_1 = h_2 = h$ for the sake of simplicity.
...
The computational complexity of the PAC learning algorithms for these classes of functions is expressed in terms of a function
...
%This will be evaluated in Sec.~\ref{sec:abinitio}.
...
%The only way to have such a guarantee would be to perform this an infinite number of time.
...
% arises as one of the first improvements that should be worked on.
...
Indeed, the activation functions in the Lokta--Volterra models report the apparition on extinction of the species' population as a whole and not of individuals of it.
...
Predator, Prey -< Prey
      \end{lstlisting}
   \end{minipage}
   \caption{The Lokta-Volterra prey vs.\ predator influence model of Ex.~\ref{ex:LVi}
...
samples for each function can be pretty non-uniform. Interestingly, the minimal number of samples gives us the lowest
...
% In our experiments about 1000 different initial states were needed to find a
% model very close to the original one.

\subsection{PAC Learning with Prior Knowledge on the Influence Graph}
\label{sec:prior}

Furthermore, to improve the guarantee $h$ and the corresponding accuracy of the learnt
