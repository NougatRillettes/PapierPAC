...
In all of those, we consider the boolean state of activation of the system, that is a vector of $\mathbb{B}^n$ that says whether or not the $i$th gene is activated, or the $i$th species present. 
...
\wip{Reprendre les definitions qui suivent.}
...
Positive influence systems may hence be represented as $n$ activation and $n$ deactivation functions, which are defined as follow:
...
A peculiarly fast-thinking reader may also have notice that different (and often infinitely many) states of the system may also be abstracted by the same boolean vector $v$, and that the transition may also happen from only some of them.
